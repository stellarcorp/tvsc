 ...
 * main(): determine_service_addr(SERVICE_NAME, FLAGS_end_point, FLAGS_service_configuration, "ip:port");
 *
...
inline std::string determine_service_addr(std::string_view service_name, int default_port) {
  ...
  return determine_service_addr(service_name, FLAGS_end_point, FLAGS_service_configuration, default_bind);
}
