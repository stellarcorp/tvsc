...
struct EncodedPacket final {
  ...
  tvsc::buffer::Buffer<tvsc::comms::radio::Fragment<MTU>, MAX_FRAGMENTS_PER_PACKET> buffers{};
};
...
bool encode(const PacketT& packet, EncodedPacket<MTU, MAX_FRAGMENTS_PER_PACKET>& fragments) {
  ...
  static_assert(std::is_same<uint8_t, std::underlying_type_t<tvsc::comms::radio::Protocol>>::value,
  ...
  tvsc::comms::radio::Fragment<MTU>* current_fragment{nullptr};
  ...
  for (fragment_index = 0; fragment_index < MAX_FRAGMENTS_PER_PACKET && have_more_to_encode;
    ...
    if (remaining_payload > 0) {
      ...
      const size_t fragment_payload_size =
          std::min(remaining_payload, tvsc::comms::radio::Fragment<MTU>::max_payload_size());

      current_fragment->set_payload_size(fragment_payload_size);

      current_fragment->data.write_array(
          tvsc::comms::radio::Fragment<MTU>::PAYLOAD_DATA_OFFSET, fragment_payload_size,
...
bool decode(const tvsc::comms::radio::Fragment<MTU>& fragment,
            PacketT<PACKET_MAX_PAYLOAD_SIZE>& packet) {
  static_assert(tvsc::comms::radio::Fragment<MTU>::max_payload_size() <= PACKET_MAX_PAYLOAD_SIZE,
