...
void SpiBus::initialize_peripheral(uint8_t peripheral_select_pin, uint8_t write_address_mask) {
  pinMode(peripheral_select_pin, OUTPUT);

  // Initialize the pin to deselect state.
  deselect_peripheral(peripheral_select_pin);
...
SpiPeripheralSelection SpiBus::select_peripheral(uint8_t peripheral_select_pin) {
  digitalWrite(peripheral_select_pin, LOW);

  return SpiPeripheralSelection{*this, peripheral_select_pin};
}

void SpiBus::deselect_peripheral(uint8_t peripheral_select_pin) {
...
SpiTransaction SpiBus::begin_transaction() {
