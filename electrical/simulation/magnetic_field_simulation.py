...
class MagneticFieldSimulation:
    """
    Simulates the magnetic field generated by a PCB trace using the Biot-Savart law and FEM
    discretization.
    """
    def __init__(self, trace, voltage):
        self.trace = trace
    ...
    def biot_savart(self, observation_point):
        ...
        for segment in self.trace.segments:
            ...
            dB = mu_0 * self.current * np.cross(dl, r) / (4 * np.pi * r_mag**3)
            B += dB
    ...
    def compute_torque(self, B_ext, center=np.array([0., 0., 0.])):
        """Computes the total force and torque on the PCB in an external magnetic field."""
        moment = self.trace.estimate_magnetic_moment(self.current)
