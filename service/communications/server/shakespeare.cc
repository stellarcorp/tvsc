...
size_t Shakespeare::get_next_line(unsigned char* dest, size_t capacity) {
  ...
  if (newline_position == full_text_.end()) {
    ...
    newline_position = std::find(position_, full_text_.end(), '\n');
  }

  // We add one to make sure that we get the newline in the result.
  ++newline_position;

  size_t i = 0;
  for (auto iter = position_; iter != newline_position && i < capacity; ++iter, ++i) {
    *(dest + i) = *iter;
  }

  // If we don't have the capacity to get the full line, we still just forward to the start of the
  // next line. Just makes for better sample data.
