...
TEST(ComputeMaskTest, GeneratesDesiredMask8bit) {
  ...
  uint32_t mask{};
  mask = bits::compute_bit_mask<8, 0>();
  EXPECT_EQ(0b11111111, mask);
  mask = bits::compute_bit_mask<6, 2>();
  EXPECT_EQ(0b11111100, mask);
  mask = bits::compute_bit_mask<1, 1>();
  EXPECT_EQ(0b00000010, mask);
  mask = bits::compute_bit_mask<1, 0>();
  EXPECT_EQ(0b00000001, mask);
  mask = bits::compute_bit_mask<1, 6>();
  EXPECT_EQ(0b01000000, mask);
  mask = bits::compute_bit_mask<3, 4>();
  EXPECT_EQ(0b01110000, mask);
}

TEST(ComputeMaskTest, GeneratesDesiredMask) {
  uint32_t mask{};
  mask = bits::compute_bit_mask<32, 0>();
  EXPECT_EQ(static_cast<uint32_t>(-1), mask);
  mask = bits::compute_bit_mask<31, 0>();
  EXPECT_EQ(static_cast<uint32_t>(-1) >> 1, mask);
  mask = bits::compute_bit_mask<31, 1>();
  EXPECT_EQ(static_cast<uint32_t>(-1) << 1, mask);
  mask = bits::compute_bit_mask<3, 11>();
  EXPECT_EQ(0b00000000000000000011100000000000, mask);
  mask = bits::compute_bit_mask<8, 16>();
...
TEST(ComputeMaskTest, CanComputeMaskAtCompile) {
  ...
  static constexpr uint32_t mask{bits::compute_bit_mask<3, 4>()};
