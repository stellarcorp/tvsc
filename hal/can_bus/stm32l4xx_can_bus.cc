...
void CanBusStm32l4xx::enable() {
  ...
  can_bus_.Init.Prescaler = 4;           // Divides PCLK1 (e.g. 32 MHz) to get tq clock (e.g. 8 MHz)
  ...
  can_bus_.Init.AutoWakeUp =
      DISABLE;  // Wakeup via CAN bus activity (not needed unless using sleep)
  ...
  // TODO(james): Enable interrupts here, using a call like that commented out below, and in the relevant boards.
...
bool CanBusStm32l4xx::data_available(RxFifo fifo) {
  return HAL_CAN_GetRxFifoFillLevel(&can_bus_, CAN_RX_FIFO0 + cast_to_underlying_type(fifo)) > 0;
}
