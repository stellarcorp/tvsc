...
TEST(TelemetryAccumulatorTest, ReportOnInitializedIncludesTimeEvent) {
  ...
  const tvsc_radio_nano_TelemetryEvent* time_event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_TIME)};
...
TEST(TelemetryAccumulatorTest, ReportOnInitializedIncludesAllOwnedMetrics) {
  ...
  for (auto metric : {tvsc_radio_nano_Metric_PACKETS_RX, tvsc_radio_nano_Metric_PACKETS_TX,
                      tvsc_radio_nano_Metric_DROPPED_PACKETS, tvsc_radio_nano_Metric_TX_ERRORS}) {
    const tvsc_radio_nano_TelemetryEvent* event{TelemetryAccumulator::find_event(report, metric)};
...
TEST(TelemetryAccumulatorTest, CanIncrementPacketsReceived) {
  ...
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_PACKETS_RX)};
...
TEST(TelemetryAccumulatorTest, CanIncrementPacketsTransmitted) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_PACKETS_TX)};
...
TEST(TelemetryAccumulatorTest, CanIncrementPacketsDropped) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_DROPPED_PACKETS)};
...
TEST(TelemetryAccumulatorTest, CanIncrementTransmitErrors) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_TX_ERRORS)};
...
TEST(TelemetryAccumulatorTest, CanReportPower) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_POWER_W)};
...
TEST(TelemetryAccumulatorTest, CanReportRssi) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_RSSI_DBM)};
...
TEST(TelemetryAccumulatorTest, CanReportTxQueueSize) {
  ...
  const tvsc_radio_nano_TelemetryReport report{telemetry.generate_telemetry_report()};
  const tvsc_radio_nano_TelemetryEvent* event{
      TelemetryAccumulator::find_event(report, tvsc_radio_nano_Metric_TX_QUEUE_SIZE)};
