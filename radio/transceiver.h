...
class BestEffortTransceiver final {
 private:
 ...
 public:
  BestEffortTransceiver() : configuration_(radio_, SingleRadioPinMapping::board_name()) {
    configuration_.change_values(default_configuration<RadioT>());
    configuration_.commit_changes();
  }

  BestEffortTransceiver(const tvsc_radio_nano_RadioIdentification& identification)
