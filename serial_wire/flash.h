 ...
 public:
  ...
  [[nodiscard]] Result write_flash(uint32_t address, const uint32_t* source, uint32_t length) {
    ...
    for (current_page = 0; success && current_page < num_pages; ++current_page) {
      // Set the page number, enable end of operation (EOP) interrupts, and set the programming bit
      // (PG).
      // Set the page number and set the programming bit (PG).
      if (success) {
        success = target_->ap_read_mem(FLASH_CR, flash_ctrl);
      }
      if (success) {
        ...
        // bits::modify_bit_field<1, 24>(flash_ctrl, 1UL);
        success = target_->ap_write_mem(FLASH_CR, flash_ctrl);
      }

      if (success) {
        success = target_->ap_write_mem(
            address + current_page * FLASH_PAGE_SIZE,
            source + (current_page * FLASH_PAGE_SIZE) / sizeof(uint32_t), FLASH_PAGE_SIZE);
      }
      if (success) {
        do {
          success = target_->ap_read_mem(FLASH_SR, flash_status);
